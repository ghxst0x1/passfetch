...
fn main() {
    ...
    let local_state_path = PathBuf::from(&user_profile).join("Google/Chrome Beta/User Data/Local State");
    let login_data_path =
        PathBuf::from(&user_profile).join("Google/Chrome Beta/User Data/Default/Login Data");
